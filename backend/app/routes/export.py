...
async def export_csv():
    ...
    for t in txns:
        writer.writerow([
            t.get("date", ""), t.get("description", ""), t.get("amount", 0),
...
async def export_excel():
    ...
    for r, t in enumerate(txns, start=1):
        ...
        ws.write_number(r, 2, float(t.get("amount", 0)))
        ws.write(r, 3, t.get("category", ""))
        ws.write(r, 4, t.get("payment_method", ""))
        ws.write(r, 5, t.get("notes", ""))
    wb.close()
    out.seek(0)
    return StreamingResponse(
        out,
